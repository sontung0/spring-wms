...
class UserEventConsumer {
    ...
    Consumer<UserCreatedEvent> sendUserCreatedNoti() {
        return event -> log.info("Sending notification for created user: {}", event.user().getEmail());
    }

    @Bean
    Consumer<UserUpdatedEvent> sendUserUpdatedNoti() {
        return event -> log.info("Sending notification for updated user: {}", event.user().getEmail());
    }
}
